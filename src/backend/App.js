...
app.get('/api/professor/testes/:id/resultados', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const [questoes] = await pool.query(
            'SELECT q.id, q.enunciado FROM teste_questoes tq JOIN questoes q ON tq.questao_id = q.id WHERE tq.teste_id = ?',
            [id]
        );

        // Obter todos os alunos que responderam o teste
        const [alunos] = await pool.query(
            'SELECT DISTINCT a.id, a.nome, a.numero FROM respostas r JOIN alunos a ON r.aluno_id = a.id WHERE r.teste_id = ?',
            [id]
        );
        ...
        for (const aluno of alunos) {
            ...
            const [respostas] = await pool.query(
                'SELECT r.questao_id, r.correta FROM respostas r WHERE r.teste_id = ? AND r.aluno_id = ?',
                [id, aluno.id]
            );
        ...
        for (const questao of questoes) {
            const [respostas] = await pool.query(
                'SELECT COUNT(*) as total, SUM(correta) as acertos FROM respostas WHERE teste_id = ? AND questao_id = ?',
                [id, questao.id]
            );
...
app.get('/api/professor/testes/:id/resultados/excel', authenticateToken, async (req, res) => {
    ...
    try {
       ...
       LEFT JOIN respostas r ON tq.questao_id = r.questao_id AND r.teste_id = tq.teste_id
       WHERE tq.teste_id = ?
    ...
    try {
        ...
        if (teste[0].tema_id || teste[0].dificuldade) {
            ...
            [questoes] = await pool.query(
                'SELECT q.* FROM teste_questoes tq JOIN questoes q ON tq.questao_id = q.id WHERE tq.teste_id = ?',
                [id]
            );
        }
...
app.post('/api/estudante/testes/:id/respostas', async (req, res) => {
    ...
    try {
        ...
        const [existing] = await pool.query(
            'SELECT * FROM respostas WHERE teste_id = ? AND aluno_id = ? LIMIT 1',
            [id, aluno_id]
        );
        ...
        for (const resposta of respostas) {
            await pool.query(
                'INSERT INTO respostas (teste_id, aluno_id, questao_id, alternativa_id, correta) VALUES (?, ?, ?, ?, ?)',
        ...
        // Calcular resultado
        const [result] = await pool.query(
            'SELECT COUNT(*) as total, SUM(correta) as acertos FROM respostas WHERE teste_id = ? AND aluno_id = ?',
            [id, aluno_id]
        );
